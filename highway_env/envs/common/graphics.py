...
class EventHandler(object):
    ...
    def handle_continuous_action_event(cls, action_type: ContinuousAction, event: pygame.event.EventType) -> None:
        ...
        if event.type == pygame.JOYHATMOTION:
            if event.value == (0, -1) and action_type.lateral:
                action[steering_index] = 0.15  # steer up heading
            if event.value == (0, 1) and action_type.lateral:
                action[steering_index] = -0.15  # steer down heading
            if event.value == (0, 0):
                action[steering_index] = 0

        elif event.type == pygame.JOYBUTTONDOWN:
            if event.button == 1:
                action[0] = 0.2  # increase vel
            if event.button == 3:
                action[0] = -0.2  # decrease vel
        # cisimon7
        # print(action)
