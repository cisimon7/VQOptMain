...
class RoadObject(ABC):
    ...
    def polygon(self) -> np.ndarray:
        ...
        # self.heading = 0 if np.isnan(self.heading) else self.heading  # CISIMON7
        ...
        # CISIMON7
        # self.position = np.asarray([ 0 for _ in self.position]) if np.all(np.isnan(self.position)) else self.position 
